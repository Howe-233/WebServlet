                                ...
                                <ul class="nav nav-subnav">
                                    <c:if test="${not empty menu.list}">
                                        ...
                                        <c:forEach var="menuItem" items="${menu.list}">
                                            <li
                                                    <c:if test="${menuItem.id == 0}"> class="active" </c:if>
                                            >
                                                <a
...
<%--                                    <button class="btn btn-default"><i class="mdi mdi-magnify"></i></button>--%>
                                ...
                                <div class="toolbar-btn-action">
                                    ...
                                    <a class="btn btn-success m-r-5" href="#!" onclick="delBusinessInfo()"><i
                            ...
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table class="table table-bordered">
                                        ...
                                        <tbody>
                                        <c:forEach var="businessInfo" items="${businessInfoAll}">
                                            <tr>
                                                <td>
                                                    <label class="lyear-checkbox checkbox-primary">
                                                        <input type="checkbox" id="delId" name="ids[]"
                                                               value="${businessInfo.id}"><span></span>
                                                    </label>
                                                </td>
                                                <td>${businessInfo.id}</td>
                                                <td>${businessInfo.storeName}</td>
                                                <td>${businessInfo.notes}</td>
                                                <td>${businessInfo.tel}</td>
                                                <td>${businessInfo.address}</td>
                                                <td>${businessInfo.avatar}</td>

                                                <td>
                                                    <fmt:formatDate value="${businessInfo.createTime}"
                                                ...
                                                <td>
                                                    <div class="btn-group">
                                                        <a data-toggle="modal"
                                                           ...
                                                           onclick="modifyBusinessInfo(${businessInfo.id})"
                                                        ><i
                                                                class="mdi mdi-pencil"></i></a>
                                                        <c:if test="${businessInfo.status == 1}">
                                                            <a class="btn btn-xs btn-default" title="删除"
                                                               data-toggle="tooltip"
                                                               onclick="delBusinessOne(${businessInfo.id})"><i
                    ...
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            <div class="modal-header">
                                ...
                                <h4 class="modal-title" id="exampleModalLabel">商家信息修改
                                </h4>
                            </div>
                            <div class="modal-body">
                                <form>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control"
                                               id="businessId" readonly>
                                    </div>
                                    <div class="form-group">
                                        <label for="storeName"
                                               class="control-label">店铺名称</label>
                                        <input type="text" class="form-control"
                                               id="storeName">
                                    </div>
                                    <div class="form-group">
                                        <label for="notes"
                                               class="control-label">主营</label>
                                        <input type="text" class="form-control"
                                               id="notes">
                                    </div>
...
<script type="text/javascript">
    function modifyBusinessInfo(id) {
        $.ajax({
            url: "${ctx}/api/business/modifyBusinessInfo",
            type: "post",
            dataType: "json",
            data: {
                id: id
            ...
            success(res) {
                if (res.code == 200) {
                   document.getElementById("businessId").value = res.data.id;
...
<script type="text/javascript">
    function saveBusiness() {
        if (document.getElementById("businessId").value == null || document.getElementById("businessId").value == "") {
            ...
            $.ajax({
                url: "${ctx}/api/business/createBusinessInfo",
                type: "post",
                dataType: "json",
                data: {
                    storeName: storeName,
                ...
                success(res) {
                    if (res.code == 200) {
                        alert("保存成功");
                        location.href = "${ctx}/api/business/showBusiness";
                    } else {
                        alert("保存失败");
                    }
                }
            ...
            let id = document.getElementById("businessId").value;
            let storeName = document.getElementById("storeName").value;
            let notes = document.getElementById("notes").value;
            let tel = document.getElementById("tel").value;
            let address = document.getElementById("address").value;
            ...
            if (confirm("确认保存吗")) {
                $.ajax({
                    url: "${ctx}/api/business/updateBusinessInfo",
                    type: "post",
                    dataType: "json",
                    data: {
                        id: id,
...
<script type="text/javascript">
    function delBusinessOne(id) {
        if (confirm("确认删除吗？")) {
            if (id == null || id == "") {
                alert("删除失败")
            } else {
                $.ajax({
                    url: "${ctx}/api/business/deleteBusinessOne",
                    type: "post",
                    dataType: "json",
                    data: {
                        id: id
