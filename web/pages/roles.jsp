...
<div class="lyear-layout-web">
    <div class="lyear-layout-container">
        ...
        <aside class="lyear-layout-sidebar">
                ...
                <nav class="sidebar-main">
                    <ul class="nav nav-drawer">
                        ...
                        <c:forEach var="menu" items="${menuList}">
                            ...
                            <li <c:if test="${not empty menu.list}">
                                class="nav-item nav-item-has-subnav
                                <c:if test="${menu.id==5}">active open</c:if>"
                            </c:if>
                            >
                                <a<c:if test="${not empty menu.href}"> href="${ctx}${menu.href}" </c:if> >
                                ...
                                <ul class=" nav nav-subnav">
                                    <c:if test="${not empty menu.list}">
                                        <c:forEach var="menuItem" items="${menu.list}">
                                            <li <c:if test="${menuItem.id==7}"> class="active"
                                            </c:if> ><a
            ...
            <%--                                        </c:if>--%>
            <%--                                    </c:if>--%>
            <%--                            ><a <c:if test="${not empty menu.href}"> href="${menu.href}" </c:if> ><i--%>
            <%--                                    class="${menu.icon}"></i>${menu.title}</a>--%>
            <%--                                <ul class="nav nav-subnav">--%>
            <%--                                    <c:if test="${not empty menu.list}">--%>

            <%--                                        <c:forEach var="menuItem" items="${menu.list}">--%>
            <%--                                            <li <c:if test="${menu.id==5}"> class="active" </c:if>>--%>
            <%--                                                <a href="${menuItem.href}">${menuItem.title} </a></li>--%>
            <%--                                        </c:forEach>--%>
                            ...
                            <div class="card-toolbar clearfix">
                                ...
                                <div class="toolbar-btn-action">
                                    <a class="btn btn-primary m-r-5" href="#!"
                                       ...
                                       onclick="document.getElementById('roleForm').reset()"
                                       ><i class="mdi mdi-plus"></i> 新增</a>
                                    <a class="btn btn-success m-r-5" href="#!" onclick="deleteInfo()"><i
                                            class="mdi mdi-check"
                                    ></i>
                                        修改状态</a>
                            ...
                            <div class="card-body">
                                ...
                                <div class="table-responsive">
                                    <table class="table table-bordered">
                                        ...
                                        <tbody>
                                        <c:forEach var="roleInfo" items="${roleInfoAll}">
                                            <tr>
                                                <td>
                                                    <label class="lyear-checkbox checkbox-primary">
                                                        <input type="checkbox" name="ids[]"
                                                               value="${roleInfo.id}"><span></span>
                                                    </label>
                                                </td>
                                                <td>${roleInfo.id}</td>
                                                <td>${roleInfo.name}</td>
                                                ...
                                                <td>
                                                    <c:if test="${roleInfo.status == 1}">
                                                        <font class="text-success">
                                                            正常
                                                        </font>
                                                    </c:if>
                                                    <c:if test="${roleInfo.status == 0}">
                                                ...
                                                <td>
                                                    <div class="btn-group">
                                                        <a data-toggle="modal"
                                                           ...
                                                           onclick="modifyRoleInfo(${roleInfo.id})"
                                                        ><i
                                                                class="mdi mdi-pencil"></i></a>
                                                        <c:if test="${roleInfo.status == 1}">
                                                            <a class="btn btn-xs btn-default" title="删除"
                                                               data-toggle="tooltip"
                                                               onclick="delRoleOne(${roleInfo.id})"><i
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title" id="exampleModalLabel">角色信息修改
                        </h4>
                    </div>
...
<script type="text/javascript">
        ...
        if (confirm("确定修改吗")) {
            $.ajax({
                ...
                success(res) {
                    if (res.code == 200) {
                        alert("修改成功");
                        location.href = "${ctx}/api/role/showRole";
                    } else {
                        alert(res.msg);
                    }
                }
    ...
    //回显数据
    function modifyRoleInfo(id) {
        $.ajax({
            url: "${ctx}/api/role/modify",
            type: "POST",
            dataType: "JSON",
            data: {
                id: id
