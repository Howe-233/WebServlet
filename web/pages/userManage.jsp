    ...
    <div class="lyear-layout-container">
        ...
        <aside class="lyear-layout-sidebar">
            ...
            <div id="logo" class="sidebar-header">
                <a href="${ctx}/index.jsp"><img src="${ctx}/static/bootstrap/images/logo-sidebar.png" title="LightYear"
                                                alt="LightYear"/></a>
            </div>
                                ...
                                <ul class="nav nav-subnav">
                                    <c:if test="${not empty menu.list}">
                                        ...
                                        <c:forEach var="menuItem" items="${menu.list}">
                                            <li
                                                    <c:if test="${menuItem.id == 7}"> class="active" </c:if>
                                            >
                                                <a
            ...
            <nav class="navbar navbar-default">
                <div class="topbar">
                    ...
                    <div class="topbar-left">
                        ...
                        <span class="navbar-page-title"> 用户管理页面 </span>
                    </div>
                    ...
                    <div class="col-lg-12">
                        <div class="card">
                            <div class="card-toolbar clearfix">
                                <form class="pull-right search-bar" action="${ctx}/api/user/userManage" role="form">
                                ...
                                <div class="toolbar-btn-action">
                                    <a class="btn btn-primary m-r-5" href="addUser"><i class="mdi mdi-plus"></i>
                                        新增</a>
                                    <a class="btn btn-success m-r-5" onclick="delUser()"><i class="mdi mdi-check"></i>修改状态</a>
                                    <%--                                    <a class="btn btn-warning m-r-5" href="#!"><i class="mdi mdi-block-helper"></i> 禁用</a>--%>
                                    <%--                                    <a class="btn btn-danger" onclick="delUser()"><i class="mdi mdi-window-close"></i>--%>
                                    <%--                                        删除</a>--%>
                                </div>
                            </div>
                            <div class="card-body">
                                ...
                                <div class="table-responsive">
                                    <table class="table table-bordered">
                                        ...
                                        <tbody>
                                        <c:forEach var="userInfo" items="${allUserInfoList}">
                                            <tr>
                                                <td>
                                                    <label class="lyear-checkbox checkbox-primary">
                                                        <input type="checkbox" id="delId" name="ids[]"
                                                               value="${userInfo.id}"><span></span>
                                                    </label>
                                                </td>
                                                <td>${userInfo.id}</td>
                                                <td>${userInfo.name}</td>
                                                ...
                                                <td>
                                                    <div class="btn-group">
                                                        <a data-toggle="modal"
                                                           ...
                                                           onclick="modifyUserInfo(${userInfo.id})"
                                                        ><i class="mdi mdi-pencil"></i></a>

                                                        <c:if test="${userInfo.status == 1}">
                                                            <a class="btn btn-xs btn-default" title="删除"
                                                               data-toggle="tooltip"
                                                               onclick="delUserOne(${userInfo.id})"><i
                                ...
                                <hr>
                                <div name="modify" id="modify">
                                    <%--                                    <button type="button" class="btn btn-primary" data-toggle="modal"--%>
                                    <%--                                            data-target="#exampleModal" data-whatever="@mdo">打开模态框 for @mdo--%>
                                    <%--                                    </button>--%>
                                    ...
                                    <%--                                                                   class="control-label">收件人：</label>--%>
                                    <%--                                                            <input type="text" class="form-control" id="recipient-name">--%>
                                    <%--                                                        </div>--%>
                                    <%--                                                        <div class="form-group">--%>
                                    <%--                                                            <label for="message-text"--%>
                                    <%--                                                                   class="control-label">消息内容：</label>--%>
                                        ...
                                        <div class="modal-dialog" role="document">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    ...
                                                    <h4 class="modal-title" id="exampleModalLabel">用户信息修改
                                                    </h4>
                                                </div>
                                                <div class="modal-body">
                                                    <form>
...
<script type="text/javascript">
    ...
    function delUser() {
        let delId = document.getElementsByName("ids[]");
        let selectId = [];
        for (let i = 0; i < delId.length; i++) {
            if (delId[i].checked) {
                selectId.push(delId[i].value);

            }
        }
        if (delId.length !== 0) {
            $.ajax({
                url: "${ctx}/api/user/deleteUserInfo",
                type: "post",
                dataType: "json",
                data: {
                    selectId: selectId
                ...
                success(res) {
                    if (res.code == 200) {
                        alert("修改成功");
                        window.location.href = "${ctx}/api/user/userManage";
                    } else {
                        alert("修改失败" + res.msg);
                    }
                }
...
<script type="text/javascript">
    function modifyUserInfo(id) {
        $.ajax({
            url: "${ctx}/api/user/modifyUserInfo",
            type: "POST",
            dataType: "JSON",
            data: {
                id: id
    ...
    function saveMenu() {
        ...
        if (passwordCheck(password, uPassword)) {
            $.ajax({
                url: "${ctx}/api/user/userUpdate",
                type: "post",
                dataType: "json",
                data: {
                    id: id,
    ...
    function passwordCheck(password, uPassword) {
        if (password !== null && uPassword !== null) {
